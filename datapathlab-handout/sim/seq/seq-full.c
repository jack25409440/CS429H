...
int gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVL) || (icode) == 
        (I_MRMOVL) || (icode) == (I_IADDL) || (icode) == (I_MRIMOVL)) ? 
...
int gen_dstE()
{
    return ((((icode) == (I_RRMOVL)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVL) || (icode) == (I_ALU) || (icode) == (I_IADDL) || (icode)
         == (I_MRIMOVL)) ? (rb) : ((icode) == (I_PUSHL) || (icode) == 
...
int gen_dstM()
{
    return (((icode) == (I_MRMOVL) || (icode) == (I_POPL) || (icode) == 
        (I_MRIMOVL)) ? (ra) : ((icode) == (I_LEAVE)) ? (REG_EBP) : 
      (REG_NONE));
}
...
int gen_aluA()
{
    return (((icode) == (I_RRMOVL) || (icode) == (I_ALU)) ? (vala) : (
        ...
        (I_POPL) || (icode) == (I_LEAVE) || (icode) == (I_MRIMOVL)) ? 4 : 0)
    ;
}
...
int gen_aluB()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_MRMOVL) || (icode) == 
        ...
        (icode) == (I_LEAVE) || (icode) == (I_MRIMOVL)) ? (valb) : ((icode)
...
{
    return ((icode) == (I_MRMOVL) || (icode) == (I_POPL) || (icode) == 
      (I_RET) || (icode) == (I_LEAVE) || (icode) == (I_MRIMOVL));
}
...
int gen_mem_addr()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_PUSHL) || (icode) == 
        (I_CALL) || (icode) == (I_MRMOVL)) ? (vale) : ((icode) == (I_POPL)
         || (icode) == (I_RET) || (icode) == (I_LEAVE)) ? (vala) : ((icode)
         == (I_MRIMOVL)) ? (valb) : 0);
}
