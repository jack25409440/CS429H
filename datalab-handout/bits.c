...
unsigned float_i2f(int x) {
  ...
  {  
     ...
     x = (x >> 9)&mask;    // keep it in 23 bits
     index=31-e;           // calculate the how many bits the point shift
     e = (158 - e) << 23;  // since left shift, the actual exponent is 158-e
     if(index>23)
    {
         threshold=1<<(index-24);                  //calculate the threshold for rounding
         truncate=originx&((threshold<<1)-1);      //calculate the value truncated
    }
   result=((sign | e) | x);
     /*! When the value truncated is greater than the threshold, round up */
     if(truncate>threshold)
     {
      result=result+1;
     }
     /*! when truncate!=0 and is equal to threshold, round half-way */
