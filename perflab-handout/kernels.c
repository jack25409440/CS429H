 ...
 */
team_t team = {
    "Xiaohui Chen",              /* Team name */

    "Xiaohui Chen(xc2388)",     /* First member full name */
    "xhchen0328@utexas.edu",  /* First member email address */

    "",                   /* Second member full name (leave blank if none) */
    ""                    /* Second member email addr (leave blank if none) */
};
/*
 * The name of the CS machine get the follwing best performance: xc2388
 * Summary of Your Best Scores:
 * Rotate: 2.2 (rotate: Current working version)
	...
	for(i=0;i<dim;i+=32)
        {
           for(j=dim-1;j>=0;j--)
            {
             pixel *dest=dst+RIDX(dim-1-j,i,dim);
             pixel *sour=src+RIDX(i,j,dim);
             *dest++=*sour;
             sour+=dim;
             *dest++=*sour;
             sour+=dim;
...
void smooth(int dim, pixel *src, pixel *dst) 
{
	...
	dst++;
        ...
        //case2: row2 to the second last row(Center position)
	pixelA = src;
	pixelB = pixelA + dim;
	pixelC = pixelB + dim;
	for(i = 1; i < size; i++)
	{
		//the first pixel of line i	
