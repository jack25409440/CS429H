...
#PC update   PC<----valP               #
########################################
#iaddl helps me save 1 cycle when adding
#a constant to a register

#Modification of branch prediction:
#I declared a variable UNCOND to check if 
#the jumps are conditional. Unconditional
#jumps and call instruction will not have
#misprediction check. As for conditional
#jump, branch will be taken when valC<valP.
#Otherwise, branch will not taken. Besides
#modifying PC prediction, PC fetch and conditions
#of D_bubble and E_bubble, I modify IJXX to set E_valE
#to ValC and so comparation can be taken in execute 
#stage. This strategy is useful for loop initiation
#since loops nearly non-taken in most case.


#Modification of bypassing
#When load/use hazard occurs and the load 
#instruction's destination register will 
#be used as a source register, bypassing 
#can replace stall. In this case, while 
#load instruction is in memory stage, m_valM
#will be forwarded to e_valA. I modified conditions
#for geting e_valA and for stalling and so bypassing
#is implemented. For each load/use hazard, assume 50%
#of the instructions can use bypass. Because there is
#1 additional penalty for stall, 2-1*0.5-2*0.5=0.5 cycles
#will be saved.
...
int e_valA = [
        M_dstM==E_srcA && M_icode in { IMRMOVL, IPOPL } : m_valM; 
